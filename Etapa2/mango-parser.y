...
void execute_assignment_logic(
    Attributes* lhs_attr,                   // Attributes of the left-hand side (variable or array element)
    int assignment_op_type,                 // 0 for =, 1 for +=, 2 for -=, 3 for *=
    const ExpresionAttribute& rhs_expr,     // The evaluated right-hand side expression
    const std::string& lhs_name,            // Name of the LHS (for error messages)
    SymbolTable& s_table                    // Symbol table reference
) {
    string lhs_declared_type_name = lhs_attr->type->symbol_name;

    // For compound assignments, LHS must be initialized
    if (assignment_op_type != 0) { // If not simple assignment (+=, -=, *=)
        if (std::holds_alternative<std::nullptr_t>(lhs_attr->value)) {
            ERROR_TYPE = NON_DEF_VAR;
            yyerror(("Variable/Elemento '" + lhs_name + "' no inicializada antes de usarla en operación '" + 
                     std::string(assignment_op_type == 1 ? "+=" : (assignment_op_type == 2 ? "-=" : "*=")) + "'.").c_str());
            YYABORT;
        }
    }

    switch (assignment_op_type) {
        case 0: // Simple Assignment (=)
            if (lhs_declared_type_name == "mango") {
                ...
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a mango '" + lhs_name + "'").c_str()); YYABORT;}
                ...
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a manguita '" + lhs_name + "'").c_str()); YYABORT;}
                ...
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a manguangua '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "tas_claro") {
                if (rhs_expr.type == ExpresionAttribute::BOOL) { lhs_attr->value = (bool)rhs_expr.ival; } // Assuming ival holds bool for ExpresionAttribute
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a tas_claro '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "higuerote") {
                if (rhs_expr.type == ExpresionAttribute::STRING) { lhs_attr->value = std::string(rhs_expr.sval); }
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a higuerote '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "negro") {
                if (rhs_expr.type == ExpresionAttribute::CHAR) { lhs_attr->value = rhs_expr.cval; }
                else { ERROR_TYPE = TYPE_ERROR; yyerror(("Asignación inválida: no se puede asignar " + std::string(typeToString(rhs_expr.type)) + " a negro '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (rhs_expr.type == ExpresionAttribute::ID) { // RHS is an identifier
                Attributes *rhs_id_attr = s_table.search_symbol(rhs_expr.sval);
                if (rhs_id_attr) {
                    if (rhs_id_attr->category == VARIABLE || rhs_id_attr->category == CONSTANT) {
                        if (std::holds_alternative<std::nullptr_t>(rhs_id_attr->value) && rhs_id_attr->type->symbol_name != "pointer") {
                             ERROR_TYPE = NON_DEF_VAR; yyerror(("Variable '" + std::string(rhs_id_attr->symbol_name.c_str()) + "' usada en asignación antes de ser inicializada.").c_str()); YYABORT;
                        }
                        // Type compatibility for ID assignment
                        if (lhs_declared_type_name == rhs_id_attr->type->symbol_name) {
                            lhs_attr->value = rhs_id_attr->value;
                        } else if (lhs_declared_type_name == "manguita" && rhs_id_attr->type->symbol_name == "mango") {
                            lhs_attr->value = static_cast<float>(std::get<int>(rhs_id_attr->value));
                        } else if (lhs_declared_type_name == "manguangua" && rhs_id_attr->type->symbol_name == "mango") {
                            lhs_attr->value = static_cast<double>(std::get<int>(rhs_id_attr->value));
                        } else if (lhs_declared_type_name == "manguangua" && rhs_id_attr->type->symbol_name == "manguita") {
                            lhs_attr->value = static_cast<double>(std::get<float>(rhs_id_attr->value));
                        } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Error de tipo: no se puede asignar " + std::string(rhs_id_attr->type->symbol_name.c_str()) + " a " + lhs_declared_type_name + " '" + lhs_name + "'").c_str()); YYABORT;}
                    } else { ERROR_TYPE = TYPE_ERROR; yyerror(("El lado derecho de la asignación ('" + std::string(rhs_expr.sval) + "') no es una variable o constante.").c_str()); YYABORT;}
                } else { ERROR_TYPE = NON_DEF_VAR; yyerror(("Variable '" + std::string(rhs_expr.sval) + "' no definida para asignación.").c_str()); YYABORT;}
            } else {
                ERROR_TYPE = TYPE_ERROR; yyerror(("Tipo de asignación no soportado para variable '" + lhs_declared_type_name + "' desde expresión '" + std::string(typeToString(rhs_expr.type)) + "'").c_str()); YYABORT;
            }
            break;

        case 1: // Compound Assignment (+=)
            if (lhs_declared_type_name == "mango") {
                if (rhs_expr.type == ExpresionAttribute::INT && std::holds_alternative<int>(lhs_attr->value)) {
                    lhs_attr->value = std::get<int>(lhs_attr->value) + rhs_expr.ival;
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '+=' inválida: no se puede sumar " + std::string(typeToString(rhs_expr.type)) + " a mango '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguita") {
                if (std::holds_alternative<float>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::FLOAT) { lhs_attr->value = std::get<float>(lhs_attr->value) + rhs_expr.fval; }
                    else if (rhs_expr.type == ExpresionAttribute::INT) { lhs_attr->value = std::get<float>(lhs_attr->value) + static_cast<float>(rhs_expr.ival); }
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '+=' inválida: no se puede sumar " + std::string(typeToString(rhs_expr.type)) + " a manguita '" + lhs_name + "'").c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguita '" + lhs_name + "' no contiene un valor flotante para '+='.").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguangua") {
                 if (std::holds_alternative<double>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::DOUBLE) { lhs_attr->value = std::get<double>(lhs_attr->value) + rhs_expr.dval; }
                    else if (rhs_expr.type == ExpresionAttribute::FLOAT) { lhs_attr->value = std::get<double>(lhs_attr->value) + static_cast<double>(rhs_expr.fval); }
                    else if (rhs_expr.type == ExpresionAttribute::INT) { lhs_attr->value = std::get<double>(lhs_attr->value) + static_cast<double>(rhs_expr.ival); }
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '+=' inválida: no se puede sumar " + std::string(typeToString(rhs_expr.type)) + " a manguangua '" + lhs_name + "'").c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguangua '" + lhs_name + "' no contiene un valor double para '+='.").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "higuerote") { // String concatenation
                if (rhs_expr.type == ExpresionAttribute::STRING && std::holds_alternative<std::string>(lhs_attr->value)) {
                    lhs_attr->value = std::get<std::string>(lhs_attr->value) + std::string(rhs_expr.sval);
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '+=' inválida: no se puede concatenar " + std::string(typeToString(rhs_expr.type)) + " a higuerote '" + lhs_name + "'").c_str()); YYABORT;}
            }
        ...
        case 2: // Compound Assignment (-=)
            if (lhs_declared_type_name == "mango") {
                if (rhs_expr.type == ExpresionAttribute::INT && std::holds_alternative<int>(lhs_attr->value)) {
                    lhs_attr->value = std::get<int>(lhs_attr->value) - rhs_expr.ival;
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '-=' inválida: no se puede restar " + std::string(typeToString(rhs_expr.type)) + " de mango '" + lhs_name + "'").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguita") {
                if (std::holds_alternative<float>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::FLOAT) { lhs_attr->value = std::get<float>(lhs_attr->value) - rhs_expr.fval; }
                    else if (rhs_expr.type == ExpresionAttribute::INT) { lhs_attr->value = std::get<float>(lhs_attr->value) - static_cast<float>(rhs_expr.ival); }
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '-=' inválida: no se puede restar " + std::string(typeToString(rhs_expr.type)) + " de manguita '" + lhs_name + "'").c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguita '" + lhs_name + "' no contiene un valor flotante para '-='.").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguangua") {
                 if (std::holds_alternative<double>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::DOUBLE) { lhs_attr->value = std::get<double>(lhs_attr->value) - rhs_expr.dval; }
                    ...
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '-=' inválida: no se puede restar " + std::string(typeToString(rhs_expr.type)) + " de manguangua '" + lhs_name + "'").c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguangua '" + lhs_name + "' no contiene un valor double para '-='.").c_str()); YYABORT;}
            }
            else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '-=' no soportada para el tipo '" + lhs_declared_type_name + "' de la variable '" + lhs_name + "'").c_str()); YYABORT;}
            break;
        case 3: // Compound Assignment (*=)
            if (lhs_declared_type_name == "mango") {
                if (rhs_expr.type == ExpresionAttribute::INT && std::holds_alternative<int>(lhs_attr->value)) {
                    lhs_attr->value = std::get<int>(lhs_attr->value) * rhs_expr.ival;
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '*=' inválida: no se puede multiplicar mango '" + lhs_name + "' por " + std::string(typeToString(rhs_expr.type))).c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguita") {
                if (std::holds_alternative<float>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::FLOAT) { lhs_attr->value = std::get<float>(lhs_attr->value) * rhs_expr.fval; }
                    else if (rhs_expr.type == ExpresionAttribute::INT) { lhs_attr->value = std::get<float>(lhs_attr->value) * static_cast<float>(rhs_expr.ival); }
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '*=' inválida: no se puede multiplicar manguita '" + lhs_name + "' por " + std::string(typeToString(rhs_expr.type))).c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguita '" + lhs_name + "' no contiene un valor flotante para '*='.").c_str()); YYABORT;}
            } else if (lhs_declared_type_name == "manguangua") {
                 if (std::holds_alternative<double>(lhs_attr->value)) {
                    if (rhs_expr.type == ExpresionAttribute::DOUBLE) { lhs_attr->value = std::get<double>(lhs_attr->value) * rhs_expr.dval; }
                    ...
                    else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '*=' inválida: no se puede multiplicar manguangua '" + lhs_name + "' por " + std::string(typeToString(rhs_expr.type))).c_str()); YYABORT;}
                } else { ERROR_TYPE = TYPE_ERROR; yyerror(("Variable manguangua '" + lhs_name + "' no contiene un valor double para '*='.").c_str()); YYABORT;}
            }
            else { ERROR_TYPE = TYPE_ERROR; yyerror(("Operación '*=' no soportada para el tipo '" + lhs_declared_type_name + "' de la variable '" + lhs_name + "'").c_str()); YYABORT;}
            break;
